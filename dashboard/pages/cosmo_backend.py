...
class COSMO:
    ...
    @staticmethod
    def get_train():
        return pd.read_csv(os.path.join(FILE_PATH, '/OutlierFree.csv'), index_col=0)
    ...
    @staticmethod
    def get_COS():
        model = joblib.load(os.path.join(FILE_PATH, '/models/COS.joblib'))
        return model

    @staticmethod
    def get_MO():
        model = joblib.load(os.path.join(FILE_PATH, '/models/MO.joblib'))
        return model

    @staticmethod
    def get_type_decoder():
        le = joblib.load(os.path.join(FILE_PATH, '/models/PlanetLabelEncoder.joblib'))
        return le
